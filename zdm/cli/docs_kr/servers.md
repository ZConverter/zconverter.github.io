...
**Linux Partition 정보:**
- Mount Point (예: /, /home, /var)
- Total Size
- Used Space
- Available Space
- File System Type

**Windows Partition 정보:**
- Drive Letter (예: C:, D:, E:)
- Total Size
- Used Space
- Available Space
- File System Type
...
<details markdown="1" open>
<summary><strong>백업 대상 서버 확인</strong></summary>

```bash
# 백업 가능한 모든 서버 확인
zdm-cli server list --mode source --license-assign-only --output table

# 특정 OS의 백업 대상 서버
zdm-cli server list --os lin --mode source --license-assign-only

# Partition 정보와 함께 확인
zdm-cli server list --mode source --license-assign-only --partition
```

</details>

<details markdown="1" open>
<summary><strong>복구 대상 서버 확인</strong></summary>

```bash
# 복구 가능한 Target 서버 확인
zdm-cli server list --mode target

# 특정 서버의 Partition 구조 확인
zdm-cli server list --name recovery-01 --partition-only
```

</details>

<details markdown="1" open>
<summary><strong>라이센스 미할당 서버 확인</strong></summary>
...
# OS별 라이센스 미할당 서버
zdm-cli server list --os lin --license-un-assign-only
```
...
<details markdown="1" open>
<summary><strong>서버가 목록에 표시되지 않음</strong></summary>
...
<details markdown="1" open>
<summary><strong>Partition 정보 조회 실패</strong></summary>
...
```bash
# 서버 상태 확인
zdm-cli server list --name web-server-01 --detail
...
<details markdown="1" open>
<summary><strong>라이센스 할당 안됨</strong></summary>
