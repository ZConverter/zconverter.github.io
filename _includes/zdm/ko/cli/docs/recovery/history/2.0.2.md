...
# 특정 Center의 히스토리 조회
zdm-cli recovery history --center 9

# 복수 Center 조회 (콤마 구분)
zdm-cli recovery history --center 9,10

# 작업 ID로 필터링
zdm-cli recovery history --job-id 123
...
| --server-type | - | string | Optional | - | 서버 타입 (`--server`와 함께 사용) | `source`, `target` |
...
> **참고:**
> - `--server-type`이 `target`이면 `--server`는 복구 대상(타겟) 서버를, `source`이면 소스 서버를 기준으로 필터링합니다.
...
```json
{
  ...
  "data": [
    ...
    },
    {
      ...
      "time": {
        "start": "2026-03-04 11:00:00",
        "end": "2026-03-04 11:05:00",
        "elapsed": "00:05:00"
      }
    }
  ],
