...
* 저장소 하나가 응답하지 않아도 **나머지 저장소의 이미지를 200으로 반환**합니다. 응답 스키마는 성공 시와 동일하며, 실패한 저장소는 **서버 로그에만** 기록됩니다.
* 따라서 응답만 보고는 어느 저장소가 빠졌는지 알 수 없습니다. 특정 저장소의 이미지가 보이지 않는다면 `repositoryId`로 그 저장소만 지정해 다시 호출하면 원인을 확인할 수 있습니다.
