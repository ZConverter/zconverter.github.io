...
> - `center`는 ID/이름을 콤마로 여러 개 지정할 수 있습니다. (예: `?center=1,zdm-b`) 해석되지 않는 식별자가 섞여 있으면 해석된 것만으로 조회 범위가 좁혀집니다.
...
```json
{
  ...
  "error": {
    "code": "JOB-ERROR-01",
    "message": "Backup job with Name 'daily-backup' exists, but no results match the specified filter conditions."
  },
  "timestamp": "2025-01-15T10:30:00.000+09:00"
}
```
...
`partition`과 `drive`를 함께 지정했거나, `server`/`center`를 키만 보내고 값을 비운 경우 반환됩니다.
